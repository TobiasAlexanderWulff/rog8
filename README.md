...
- Node.js 18.20+
- pnpm 10.19.0 (managed via Corepack)
...
- `pnpm typecheck` – validate types with `tsc --noEmit`
- `pnpm test` – execute the Vitest suite (see note on harness flakiness below)
- `pnpm format` / `pnpm format:check` – apply or verify Prettier formatting

> ℹ️ **Testing:** There is a known agent deadlock bug in the harness—run `pnpm test` manually only when you need verification, and jot down any flaky behaviour for follow-up. The current Vitest setup may need `--pool=forks` or serial execution when run in constrained environments; capture any issues alongside TODOs.
