...
- Use Corepack (`corepack enable` + `corepack prepare pnpm@10.19.0 --activate`), export `$HOME/.local/share/pnpm` via `GITHUB_PATH`, and restore/store the pnpm cache with `actions/cache@v4` keyed by `pnpm-lock.yaml`.
